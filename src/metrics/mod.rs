...
impl OrderBookMetrics {
    ...
    pub fn increment_trades_executed(&self, quantity: u64, notional: u64) {
        ...
        counter!("orderbook_trades_total", "status" => "executed").increment(1);
...
mod tests {
    ...
    fn test_metrics_basic_operations() {
        ...
        metrics.increment_orders_modified();
        metrics.increment_trades_executed(100, 10000);

        assert_eq!(metrics.get_orders_added(), 1);
        assert_eq!(metrics.get_orders_cancelled(), 1);
        assert_eq!(metrics.get_orders_modified(), 1);
        assert_eq!(metrics.get_trades_executed(), 1);
        assert_eq!(metrics.get_total_volume(), 100);
        assert_eq!(metrics.get_total_notional(), 10000);
    }
    ...
    fn test_latency_tracking() {
        ...
        // Simulate some work with measurable latency
        let result = metrics.time_add_order(|| {
        ...
        assert!(stats.add_order.avg_micros() >= 1000.0); // At least 1ms
    }
    ...
    fn test_concurrent_metrics() {
        ...
        for _ in 0..10 {
            ...
            let handle = thread::spawn(move || {
                for _ in 0..100 {
                    metrics_clone.increment_orders_added();
                    metrics_clone.increment_trades_executed(1, 100);
                }
            });
            handles.push(handle);
        }
        ...
        assert_eq!(metrics.get_orders_added(), 1000);
        assert_eq!(metrics.get_trades_executed(), 1000);
        assert_eq!(metrics.get_total_volume(), 1000);
        assert_eq!(metrics.get_total_notional(), 100000);
    }
}
